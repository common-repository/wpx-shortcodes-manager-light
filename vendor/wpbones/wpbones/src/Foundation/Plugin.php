...
namespace WPXShortcodesManagerLight\WPBones\Foundation;

use Closure;
use WPXShortcodesManagerLight\WPBones\Container\Container;
use WPXShortcodesManagerLight\WPBones\Contracts\Foundation\Plugin as PluginContract;
use WPXShortcodesManagerLight\WPBones\Database\WordPressOption;
use WPXShortcodesManagerLight\WPBones\Foundation\Http\Request;
use WPXShortcodesManagerLight\WPBones\Foundation\Log\LogServiceProvider;
use WPXShortcodesManagerLight\WPBones\Routing\AdminMenuProvider;
use WPXShortcodesManagerLight\WPBones\Routing\AdminRouteProvider;
use WPXShortcodesManagerLight\WPBones\Routing\API\RestProvider;
use WPXShortcodesManagerLight\WPBones\Routing\Pages\PageProvider;
use WPXShortcodesManagerLight\WPBones\Support\Str;
use WPXShortcodesManagerLight\WPBones\Support\Traits\HasAttributes;
use WPXShortcodesManagerLight\WPBones\View\View;
...
/**
 * @class   Plugin
 * @package WPXShortcodesManagerLight\WPBones\Foundation
 *
...
class Plugin extends Container implements PluginContract
{
  ...
  public function boot(): Plugin
  {
    ...
    /*
         ...
         *
         ...
         Description = "WPXShortcodesManagerLight is a WP Bones boilperate plugin"
         DomainPath = "localization"
         Name = "WPXShortcodesManagerLight"
         Network = false
         PluginURI = "http://undolog.com"
         TextDomain = "wpx-shortcodes-manager-light"
         Title = "WPXShortcodesManagerLight"
         Version = "1.0.0"
         */
   ...
   *
   * @return \WPXShortcodesManagerLight\WPBones\View\View
   */
  ...
  */
  ...
  public function js($filename, $deps = [], $version = null, $footer = true)
  {
    ...
    foreach ($filenames as $file) {
      wp_enqueue_script(
        $this->slug . Str::slug($file),
        WPXShortcodesManagerLight()->js . '/' . $file,
        (array) $deps,
        $version ?? $this->Version,
        $footer
      );
  ...
  public function getCallableHook($routes)
  {
    ...
    if (isset($controller) && isset($method)) {
      return function () use ($controller, $method) {
        $className = "WPXShortcodesManagerLight\\Http\\Controllers\\{$controller}";
